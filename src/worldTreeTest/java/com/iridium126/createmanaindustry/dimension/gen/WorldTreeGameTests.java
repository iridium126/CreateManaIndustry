...
import com.iridium126.createmanaindustry.dimension.gen.worldtree.WorldTreePalette;
...
public final class WorldTreeGameTests {
    ...
    public static void hybridTreeAndEditedCubeRoundtrip(GameTestHelper helper) throws Exception {
        ...
        var tree = new WorldTreeGenerator(level.getSeed());
        helper.assertTrue(chunk.getBlockState(new BlockPos(8, 383, 8)) == tree.sample(8, 383, 8),
        ...
        helper.assertTrue(map.generator().intersectsIsland(0, 12, 0), "Tree-only cube was filtered out");
        ...
        for (int y = 384; y < 416; y++) for (int z = 0; z < 32; z++) for (int x = 0; x < 32; x++) {
            ...
            var expected = tree.sample(x, y, z);
            ...
            helper.assertTrue(namespace.equals("minecraft") || namespace.equals("create"), "Unexpected tree material " + state);
        }
        ...
        try (var input = WorldTreeGameTests.class.getResourceAsStream("/data/createmanaindustry/markov/epic_redwood_palette.json")) {
            ...
            helper.assertTrue(new WorldTreePalette(json).state(1).is(com.simibubi.create.AllBlocks.ANDESITE_CASING.get()), "Create mapping rejected");
            json.remove("N");
            boolean rejected = false;
            try { new WorldTreePalette(json); } catch (IllegalArgumentException expected) { rejected = true; }
            helper.assertTrue(rejected, "Incomplete palette accepted");
        }
        System.out.println("WORLD_TREE_SMOKE nativeSeam=383 cubeSeam=384 comparedBlocks=32768 editedRoundtrip=ok");
        helper.succeed();
    }
}
