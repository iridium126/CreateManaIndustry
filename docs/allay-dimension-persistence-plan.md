...
为了控制改造风险，第一版允许已有持久化记录的 `getOrGenerate` 阻塞等待一次 I/O；未编辑 cube 通过 header index 可直接判定不存在，仍同步生成且不触盘。完成正确性测试后，再把玩家 shell 预取改成异步，直接交互路径保留 blocking fallback。任何异步版本都必须在服务线程完成最终对象安装。
