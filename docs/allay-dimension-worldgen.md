...
4. 原版 `applyBiomeDecoration` 在私有 `WorldGenRegion` 上执行群系的 placed/configured features。为避免玩家移动触发同步特征洪峰，每四个源区块确定性选择一个生成起点；选中的起点记录半径 1 内的修改，结果不依赖 Cube 请求顺序或缓存驱逐。所有源区块仍使用数据包群系、噪声和地表规则，树木与植被以均匀的确定性密度出现。
...
- 数据包加载和冷缓存首次生成仍比旧的简单数学场昂贵；装饰阶段采用确定性四分之一采样，并在空岛 Cube 上跳过全部噪声与群系工作。测试会记录耗时。
- 当前玩家所在 Cube 保持同步可用，周边 Cube 由单线程后台队列生成并在主线程安装；队列上限为 256，避免快速移动时积压无限世界生成任务。
- 若需要逐源区块放置全部特征，可在启动参数加入 `-Dcreatemanaindustry.allay.feature_period=1`；默认值 4 是面向正常游玩的性能配置。
